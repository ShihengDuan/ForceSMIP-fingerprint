# Slice SST to see if biases would impact detectability. 
...
def get_args():
    ...
    parser.add_argument('--pc_start', type=int, default=1950)
    parser.add_argument('--pc_end', type=int, default=2021)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    with open(path+variable+'-solver-stand-'+str(stand)+'-month-'+str(month_bool)+'-unforced-'+str(unforced)+'-joint-'+str(joint)+'-Pacific', 'wb') as pfile:
        pickle.dump(solvers, pfile)
    with open(path+variable+'-record-stand-'+str(stand)+'-month-'+str(month_bool)+'-unforced-'+str(unforced)+'-joint-'+str(joint)+'-Pacific', 'wb') as pfile:
