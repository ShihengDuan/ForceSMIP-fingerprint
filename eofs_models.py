    ...
    if month_bool and joint: # Multivariate month EOF
        ...
        for i, model in enumerate(all_models):
            ...
            for im in range(n_members):
                ...
                month_pc = solver.projectField(month_data, neofs=5).sel(time=slice(str(pc_start)+'-01-01', str(pc_end)+'-12-31'))
        ...
        for i, model in enumerate(all_models):
            ...
            model_pcs = []
            for im in range(n_members):
                ds_in = model[vid].isel(member=im).sel(time=slice(str(start_year)+'-01-01', str(2022)+'-12-31')) # this is pseudo-PC
                ds_in = ds_in.transpose('time', 'lon', 'lat')
                ds_in = ds_in * np.tile(np.expand_dims(missing_data, axis=0), (ds_in.shape[0], 1, 1))
                month_pcs = []
                for month in range(1, 13):
                    ...
                    month_pc = solver.projectField(ds_in_month, neofs=5).sel(time=slice(str(pc_start)+'-01-01', str(pc_end)+'-12-31'))
        ...
        for i, model in enumerate(all_models):
            ...
            model_pcs = []
            for im in range(n_members):
                ds_in = model[vid].isel(member=im).sel(time=slice(str(start_year)+'-01-01', str(2022)+'-12-31'))
                ds_in = ds_in.transpose('time', 'lon', 'lat')
                ds_in = ds_in * np.tile(np.expand_dims(missing_data, axis=0), (ds_in.shape[0], 1, 1))
                ds_in = ds_in - ds_in.mean(dim='time')
                pse_pc = solver.projectField(ds_in, neofs=5).sel(time=slice(str(pc_start)+'-01-01', str(pc_end)+'-12-31'))
