...
mswep = xc.open_dataset('/p/lustre3/shiduan/MSWEP/MSWEP-V280-Past-v20231102-monmaxpr-forcesmip.nc')
mswep = mswep['__xarray_dataarray_variable__'].transpose('time', 'lon', 'lat')
mswep = mswep.fillna(0)
mswep = mswep*missing_xa
mswep = mswep.sel(time=slice('1980-01-01', '2016-12-31'))
...
results_month_unforced = calculate_metrics(obs=mswep_unforced,
    solver_list=solver_list_month_unforced, 
    unforced_list=unforced_list_month_unforced, pc_series=pc_all_unforced, month=True)
del record, solver_list_month_unforced, unforced_list_month_unforced
...
results_month = calculate_metrics(obs=mswep_anomaly,
    solver_list=solver_list_month, unforced_list=unforced_list_month, pc_series=pc_all, month=True)
del solver_list_month, record, unforced_list_month
...
results_month_stand = calculate_metrics(obs=mswep_stand,
    solver_list=solver_list_month_stand, 
    unforced_list=unforced_list_month_stand, pc_series=pc_all_stand, month=True)
del record, solver_list_month_stand, unforced_list_month_stand
...
pc_list = record['pc']
results_raw= calculate_metrics(obs=mswep_anomaly,
    solver_list=solver, 
    unforced_list=unforced_list, pc_series=pc_list[0], month=False)
del record, solver, unforced_list
...
results_unforced = calculate_metrics(obs=mswep_unforced,
    solver_list=solver_list_unforced, 
    unforced_list=unforced_list_unforced, pc_series=pc_unforced[0], month=False)
del unforced_list_unforced, record, solver_list_unforced
...
path = p+variable+'-metrics-stand-False-month-True-unforced-False-joint-False-REGEN-mask'
with open(path, 'wb') as pfile:
    pickle.dump(results_month, pfile)

path = p+variable+'-metrics-stand-True-month-True-unforced-False-joint-False-REGEN-mask'
with open(path, 'wb') as pfile:
    pickle.dump(results_month_stand, pfile)

path = p+variable+'-metrics-stand-True-month-True-unforced-True-joint-False-REGEN-mask'
with open(path, 'wb') as pfile:
    pickle.dump(results_month_unforced, pfile)

path = p+variable+'-metrics-stand-False-month-False-unforced-False-joint-False-REGEN-mask'
with open(path, 'wb') as pfile:
    pickle.dump(results_raw, pfile)

path = p+variable+'-metrics-stand-True-month-False-unforced-False-joint-False-REGEN-mask'
with open(path, 'wb') as pfile:
    pickle.dump(results_stand, pfile)

path = p+variable+'-metrics-stand-True-month-False-unforced-True-joint-False-REGEN-mask'
with open(path, 'wb') as pfile:
