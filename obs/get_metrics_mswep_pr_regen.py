...
def calculate_metrics(solver_list, obs, unforced_list, pc_series, month=False, flip=False):
    ...
    if flip:
        pseudo_pc = -pseudo_pc
    print('pseudo_pc: ', pseudo_pc.shape, pseudo_pc.max().data, ' ', pseudo_pc.min().data)
    ...
    print('Noise: ', noise_pcs[0].max().data, ' ', noise_pcs[0].min().data)
    # get noise strength
...
del record, pc_unforced, unforced_list_unforced, solver_list_unforced
...
del record, solver_list_month, unforced_list_month
...
del solver_list_month_stand, record, unforced_list_month_stand
...
del record, unforced_list, solver
...
del record, solver_stand, unforced_list_stand
