...
mswep = mswep.temporal.group_average(
    '__xarray_dataarray_variable__', freq='year')
...
def calculate_metrics(solver_list, obs, unforced_list, pc_series, n_mode=1):
    ...
    # normalize pc series.
    ds_in = obs
    pseudo_pc = solver_list[0].projectField(
        ds_in-ds_in.mean(dim='time')).isel(mode=n_mode-1)
...
if not os.path.exists('/p/lustre2/shiduan/ForceSMIP/EOF/modes_all/'+str(eof_start)+'_2022_annual_cmip5/MSWEP/'):
    os.makedirs('/p/lustre2/shiduan/ForceSMIP/EOF/modes_all/' +
                str(eof_start)+'_2022_annual_cmip5/MSWEP/')
